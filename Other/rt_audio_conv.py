...
def playingCallback(in_data, frame_count, time_info, status):
    ...
    audio_frame = np.fromstring(data_bytes,dtype=np.int16)  # Convert Bytes to Numpy Array


    if wf.getnchannels() == 1:          # If Mono, duplicate to both outputs
        audio_frame_c1 = audio_frame
        audio_frame_c2 = audio_frame
    elif wf.getnchannels() == 2:        # If Stereo, un-interleave channels then split
        audio_frame = [audio_frame[idx::2] for idx in range(2)]
        audio_frame_c1 = audio_frame[0]
        audio_frame_c2 = audio_frame[1]
    else:
        raise ValueError('Something is wrong with the channel count. Num channels in WAV is not 1 or 2')
